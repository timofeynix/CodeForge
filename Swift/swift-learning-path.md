...
- Single inheritance
- Note: Property initialization order is **undefined**; it doesn't follow the declared order
- Structs let us create own custom complex data types
- To change a struct's data in a method, you must use `mutating`, and call it on a `var` instance
- Use tuples to return 2+ pieces of value from a func. They're like structs w/o a name
- Use structs for fixed data that you intend to send or receive multiple times
- Methods belong to a type (structs, enums, classes); functions do not. Swift uses the `func` to define a method. Methods can refer to other properties and methods within the type they're associated with
- Methods also help avoid namespace pollution
- **Property types**:
    - **Stored**: Value placed directly; faster access
    - **Computed**: Recalculates the property’s value every time it’s accessed
        - Must always have an explicit type
        - Constants cannot be computed properties
        - SwiftUI uses computed properties extensively
        - `get` for reading
        - `set` for writing (uses `newValue` by default)
- **Property observers**:
    - Code that runs when a property is about to be changed (`willSet`) or just after it has been changed (`didSet`)
    - Used for convenience (e.g., updating UI, validating data). Avoid heavy work here
    - Cannot be attached to `let`
    - `willSet`: Runs before the property changes (often used in SwiftUI for animations)—`newValue` can be used inside
    - `didSet`: Runs after the property changes (most commonly used)—`oldValue` can be used inside
- **Initializers**: Special functions inside structs used to set initial values for all properties
    - **Memberwise initializer**: Automatically generated by Swift if no custom initializer is provided
        - If properties have default values, they become default parameters in the memberwise initializer
        - If you create your own initializer, the memberwise initializer is removed. To keep it, use an `extension`:
            ```swift
            extension Employee {
                init() {
                    self.name = "Anonymous"
            ...
            ```
    - Initializers have no explicit return type; they always return the type they belong to
    - By the time an initializer finishes, all properties must have a value
    - Use `self` to refer to the current instance of the struct (common in initializers; required in some contexts like class closures)
